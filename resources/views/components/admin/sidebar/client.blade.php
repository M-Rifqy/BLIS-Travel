...
<aside id="sidebar" class="sidebar">
    <ul class="sidebar-nav" id="sidebar-nav">
        ...
        <li class="nav-item">
            <a class="nav-link text-14px {{ request()->routeIs('client.index') ? 'bg-primary text-white' : 'collapsed nav-link-admin' }}"
                href="{{ route('client.index') }}">
                <i class="bi bi-grid {{ request()->routeIs('client.index') ? 'text-white' : 'collapsed' }}"></i>
                <span>Dashboard</span>
            </a>
        </li><!-- Dashboard -->
